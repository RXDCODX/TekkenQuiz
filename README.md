...
## Версия сайта (SemVer + git tags)

Версия в футере теперь берется из git-тегов и приводится к SemVer:

- поддерживаются теги вида `v1.4.2` и `1.4.2`;
- если build собран ровно на теге, версия будет `1.4.2`;
- если есть коммиты после тега, версия будет формата
  `1.4.2-dev.<N>+<sha>` (например `1.4.2-dev.3+abc1234`).
...
1. Использует текущий `public/data/moves.json` (без автогенерации базы)
